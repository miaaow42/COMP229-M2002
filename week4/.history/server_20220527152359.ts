...
//     let mime_type = lookup(path.substring(1)) as string; //read the path from index 1 . path == /index.html , index 0 is /.
//     // console.log(`MIME TYPE: ${mime_type}`); //output the mime type in console to check

//     // console.log(__dirname); for test.
//     fs.readFile(__dirname + path, function(err,data){  //my dirname + request url. the function converse the url files to data, then return the data to res.end(data)
//         if(err){
...
//         res.writeHead(200,{'Content-Type': mime_type}); // response 200 status - all ok
