...
const server = http.createServer(function(req,res){
    ...
    fs.readFile(__dirname + req.url, function(err,data){  //my dirname + request url. the function converse the url files to data, then return the data to res.end(data)
        if(err){
            ...
            console.log(err.message); // output the error message to dev console
            return res.end(err.message); // return here is to stop the code running, otherwise it will display    // output the error message to the page 
        }

        // no error
        res.writeHead(200); // response 200 status - all ok
        res.end(data); // output the data (output the file that was read to page). the data comes from my dirname + request url. for example,index.html. 
        console.log(data);
    });
});
